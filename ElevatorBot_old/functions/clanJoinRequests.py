...
from ElevatorBot.core.miscFunctions import checkIfUserIsRegistered
...
async def on_clan_join_request(ctx: ComponentContext):
    ...
    for member in (await get_json_from_url(f"https://www.bungie.net/Platform/GroupV2/{CLANID}/Members/")).content[
        ...
        if memberID == destinyID:
            ...
            await ctx.send(
                hidden=True,
                embed=embed_message("Clan Application", "You are already in the clan"),
            )
            return
    ...
    embed = embed_message("Clan Application", text)
    await ctx.send(hidden=True, embed=embed)
...
async def removeFromClanAfterLeftDiscord(client, member):
    ...
    def check(reaction_reaction, reaction_user):
        return (
            (not reaction_user.bot)
            and (reaction_reaction.emoji == yes or reaction_reaction.emoji == no)
