...
async def getRasputinQuestProgress():
    ...
    return [
        (
            obHashes[objective["objectiveHash"]],
            objective["progress"],
            objective["completionValue"],
        )
        for objective in rasputinobjectives
    ]
...
async def getSpiderMaterials(discordID, destinyID, characterID):
    ...
    embed = embed_message(
        "Spider's Stock",
        description="Unlike his Fallen brethren, the stupid Spider ~~kidnapped our bird.~~ Update: He's free now",
    )
    for sale in sales.values():
        ...
        if soldname not in usermaterialreadabledict.keys():
            if "Purchase " in soldname:
                isPlural = (
                    (soldname[-1] == "s")
                    and (not soldname == "Purchase Helium Filaments")
                    and (not soldname == "Purchase Spinmetal Leaves")
                )
        ...
        embed.add_field(
            name=soldname,
            value=f"**Owned**: {ownedamount:,} {replaceWithEmote(soldname)}\n**Cost**: {pricequantity:,} {replaceWithEmote(pricename)}",
            inline=True,
        )
