...
from Backend.networking.routes import clan_get_route, clan_invite_route, clan_members_route, clan_route
...
class DestinyClan:
    ...
    async def search_clan_for_member(
        ...
        route = clan_members_route.format(
            clan_id=clan_id
        )
        params = {
        ...
        results = (await self.api.get_json_from_url(
            route=route,
            params=params
        )).content["results"]
        ...
        for result in results:
            members.append(DestinyClanMemberModel(
                ...
                last_online_status_change=datetime.datetime.strptime(result["lastOnlineStatusChange"], "%Y-%m-%dT%H:%M:%SZ"),
    ...
    async def get_clan_members(
        ...
        return await self.search_clan_for_member(
            member_name="",
            clan_id=clan_id
        )
    ...
    async def invite_to_clan(
        ...
        route = clan_invite_route.format(
            clan_id=clan_id,
            system=to_invite_system,
            destiny_id=to_invite_discord_id
        )

        welcome_message = {
            "message": f"Welcome to {clan_name}"
        }

        await self.api.post_json_to_url(
            db=self.db,
            route=route,
