...
class CRUDWeapons(CRUDBase):
    ...
    async def get_top(
        ...
        match stat:
            case stat.KILLS:
                query = query.order_by(func.sum(ActivitiesUsersWeapons.unique_weapon_kills))
            case _:
                query = query.order_by(func.sum(ActivitiesUsersWeapons.unique_weapon_precision_kills))
        query = query.desc()
