...
async def on_thread_message(client: Snake, message: Message):
    """Handles replies to user inquiries"""

    # todo no idea how this is called. Edit when events get updated
    # todo actually register the event to the handler
...
async def on_dm_message(client: Snake, message: Message):
    ...
    if not author.bot:
        ...
        if mutual_guilds:
            ...
            if mutual_allowed_guilds:
                thread_name = f"Message from {author.username}_{author.discriminator}|{author.id}"
                ...
                if author.id in reply_cache.user_to_thread:
                    thread = reply_cache.user_to_thread[author.id]
                    ...
                    if threads:
                        ...
                        await thread.send(
                            embeds=embed_message(
                                f"Inquiry from {author.display_name}",
                                "Any message you send here is relayed to them, be careful what you type :)",
                            )
                        )

                    # save in cache
                    reply_cache.user_to_thread.update({author.id: thread})
                    reply_cache.thread_to_user.update({thread: author.id})
